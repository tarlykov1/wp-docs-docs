<div class="wdl-document-list">
    <table class="wdl-document-table">
        ...
        <?php if ($data['query']->have_posts()) : while ($data['query']->have_posts()) : $data['query']->the_post();
            ...
            ?>
            <tr class="wdl-item wdl-document-item" data-search="<?php echo esc_attr(strtolower(trim(get_the_title() . ' ' . $description . ' ' . $ext . ' ' . $size_label))); ?>" data-wdl-item data-title="<?php echo esc_attr(get_the_title()); ?>" data-summary="<?php echo esc_attr($description); ?>">
                <td class="wdl-doc-thumb"><a class="wdl-doc-thumb-link" href="<?php echo esc_url(get_permalink()); ?>"><?php echo $data['helpers']->get_thumb_or_icon(get_the_ID(), $u, 'thumbnail'); ?></a></td>
                <td>
