...
class WDL_Frontend {
    ...
    public function __construct($helpers,$settings){$this->helpers=$helpers;$this->settings=$settings; add_action('wp_enqueue_scripts',array($this,'assets')); add_filter('generate_sidebar_layout',array($this,'force_no_sidebar_on_taxonomy'));  add_filter('template_include',array($this,'single_template'),99); add_filter('template_include',array($this,'taxonomy_template'),98); add_filter('the_content',array($this,'single_content_fallback'),20); add_action('template_redirect',array($this,'disable_generatepress_single_bits')); add_filter('generate_show_entry_header',array($this,'hide_generatepress_entry_header')); add_filter('generate_show_title',array($this,'hide_generatepress_title')); add_filter('generate_show_post_image',array($this,'hide_generatepress_featured_image')); add_filter('generate_featured_image_output',array($this,'hide_generatepress_featured_image_output')); add_action('wp_footer',array($this,'debug_post_type_comment')); }
    ...
    public function force_no_sidebar_on_taxonomy($layout){
